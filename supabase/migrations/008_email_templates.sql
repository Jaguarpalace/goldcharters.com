...
--
-- Every transactional email the system sends is rendered from a row in this
-- table. Editing the subject or HTML body in /admin/email-templates updates
-- the next send immediately — no code deploy needed.
--
...
values (
  'new_request_admin',
  'New valuation request — internal alert',
...
<table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#050505; padding:24px 12px;">
  ...
  jsonb_build_array(
    ...
    jsonb_build_object('key', 'logo_url', 'label', 'Brand logo URL (hosted publicly)', 'example', 'https://chartersgold.co.uk/logo/charters_gold_true_transparent.png'),
...
-- Customer-facing confirmation template.
-- Sent to the customer the moment they submit a valuation request.
insert into public.email_templates (key, name, description, subject, html_body, available_variables)
values (
